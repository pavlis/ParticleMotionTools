...
Bootstrap3DStatistics::Bootstrap3DStatistics(double *x,int nx,double confidence, int number_trials)
{
  const string base_error("Bootstrap3DStatistics constructor:  ");
  /* Sanity check */
  if(confidence>1.0)
  {
    throw SeisppError(base_error
        + "Illegal confidence interval requested - must be probability level (i.e less than 1.0)");
  }
  cl=confidence;
  int i,j,k;
  ...
  for(i=0;i<number_trials;++i)
  {
    ...
    j=3*col;
    for(k=0;k<3;++i) trials(k,i)=x[j+k];
  }
  vector<double> work;
